...
public class YUSBHub extends YGenericHub
{
	...
	{
		super(idx);
		_manager = (UsbManager) _context.getSystemService(Context.USB_SERVICE);
		if (_manager == null) {
			throw new YAPI_Exception(YAPI.IO_ERROR, "Unable to get Android USB manager");
		}
		_askPermissionIntent = PendingIntent.getBroadcast(_context, 0, new Intent(_ACTION_USB_PERMISSION), 0);
		//IntentFilter filter = new IntentFilter(_ACTION_USB_PERMISSION);
	...
	public void release()
	{
		...
		_context.unregisterReceiver(_usbBroadcastReceiver);
	}
