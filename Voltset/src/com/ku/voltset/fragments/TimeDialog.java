...
public class TimeDialog extends DialogFragment {
	...
	public String getFirstLastTimeDate() {
		...
		/*
		 * try { Logger log = new
		 * Logger(this.getActivity().getApplicationContext());
		 * log.setFile(file); FileInputStream fstream = new
		 * FileInputStream(log.getFile()); DataInputStream in = new
		 ...
		 * the console //logText.append(strLine + "\n"); }
		 * 
		 * // Close the input stream in.close(); } catch (IOException io) {
		 * io.printStackTrace(); }
		 */
		return "";
	...
	// mRoot.findViewById(R.id.tmpickerFrom);
	// TimePicker timePicker2 = (TimePicker)
	// mRoot.findViewById(R.id.tmpickerTo);
	//
